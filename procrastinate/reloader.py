...
class ChangeReloader:
    ...
    def startup_message(self) -> None:
        ...
        includes = ", ".join(self.reload_includes) if self.reload_includes else "all files"
    ...
    async def watch_files(self) -> None:
        ...
        watch_paths = [str(Path(d).resolve()) for d in self.reload_dirs if Path(d).exists()]
        ...
        try:
            async for changes in awatch(*watch_paths):
                ...
                logger.info(f"📝 Detected {len(relevant_changes)} relevant file changes:")
